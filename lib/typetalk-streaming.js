...
class TypetalkStreaming extends EventEmitter {
  constructor(options, robot) {
    ...
    this.hostname = 'typetalk.com';
    this.msgHostname = 'message.typetalk.com';
  ...
  listen() {
    const setupWebSocket = () => {
      const ws = new WebSocket(this.streamingURL || `https://${this.msgHostname}/api/v1/streaming`, {
        headers: {
  ...
  updateAccessToken(cb) {
    ...
    Request.post(options, (err, res, body) => {
      ...
      if (res.statusCode >= 400) {
        throw new Error(`Typetalk API returned unexpected status code: ${res.statusCode}`);
      }
